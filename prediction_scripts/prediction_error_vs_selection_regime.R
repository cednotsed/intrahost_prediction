...
d <- "xbb"
datasets <- c("early", "alpha", "delta", "ba1", "ba5", "xbb", "pirola")
...
morsels <- foreach(d = datasets) %do% {
  ...
  stat_df <- fread(str_glue("results/mutation_stats/{d}.stats.csv"))
  
  error_df <- res %>% 
    mutate(alias = d) %>%
    mutate(error = abs(y_test - y_pred)) %>%
    ...
    left_join(dnds_df) %>%
    left_join(stat_df %>% select(mutation_name, global_n, future_fixed, future_high))
  
  return(error_df)
}

plot_df <- bind_rows(morsels) %>%
  mutate(alias = factor(alias, c("early", "alpha", "delta", "ba1", "ba5", "xbb", "pirola")))

count_df <- plot_df %>%
  group_by(alias, class) %>%
  summarise(n = n())

plot_df %>%
  # mutate(diff = link_error - error) %>%
  ggplot(aes(x = alias, y = error, fill = class)) +
  geom_boxplot()
