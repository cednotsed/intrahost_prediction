...
file_dir <- "results/pipeline_out.020225/vcf_out.biallelic/"
file_list <- list.files(file_dir, full.names = T)

# Done
done <- list.files("results/pipeline_out.020225/consensus_out/")
done <- gsub(".fna", "", done)
...
foreach(file_name = to_do, 
  ...
  # Coverage file
  cov_df <- fread(str_glue("results/pipeline_out.020225/stats_out/{id}.coverage.txt")) %>%
  ...
  if(cov_df$coverage > 90 & !is.na(cov_df$coverage)) {
    ...
    if(nrow(counts) > 0) {
      ...
      names(dna) <- id
      writeXStringSet(dna, str_glue("results/pipeline_out.020225/consensus_out/{id}.fna"))
    }
  }  
}
