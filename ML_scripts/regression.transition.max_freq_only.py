...
### Load data
dataset = sys.argv[1]
df = pd.read_csv(datasets / f'{dataset}.max_freq_only.stats.csv')
...
test_results.to_csv(results / f'within_dataset_results/{dataset}.max_freq_only.results.csv', index=False)
...
# Export results
shap.plots.beeswarm(shap_values_ebm, show=False)
merged.to_csv(results / f'shap_out/{dataset}.max_freq_only.shap.csv', index=0)
raw_results.to_csv(results / f'results_out/{dataset}.max_freq_only.results.csv', index=0)
plt.savefig(results / f'beeswarm_out/{dataset}.max_freq_only.pdf', bbox_inches='tight')
