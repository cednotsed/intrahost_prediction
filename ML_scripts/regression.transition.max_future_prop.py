...
X = df.loc[:, ['n', 'median_freq', 'max_freq',
               ...
               'abs_hydropathy', 'delta_bind', 'delta_expr', 'mean_escape']]
...
# Logit-like transformation
p = df.loc[:, 'max_prop'].copy()
epsilon = 1e-9
y = np.log((p + epsilon) / (1 - p + epsilon))
...
test_results.to_csv(results / f'within_dataset_results/{dataset}.max_future_prop.results.csv', index=False)
...
# Export results
shap.plots.beeswarm(shap_values_ebm, show=False)
merged.to_csv(results / f'shap_out/{dataset}.max_future_prop.shap.csv', index=0)
raw_results.to_csv(results / f'results_out/{dataset}.max_future_prop.results.csv', index=0)
plt.savefig(results / f'beeswarm_out/{dataset}.max_future_prop.pdf', bbox_inches='tight')
