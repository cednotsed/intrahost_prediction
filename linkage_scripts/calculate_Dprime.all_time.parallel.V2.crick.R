...
parsed_agg <- fread(str_glue("results/allele_frequency_out/observed_out/all_time.aggregate.csv"))
...
mat_filt <- mat[, colnames(mat) %in% savs]
...
# Get pAB
cl <- makeCluster(8, outfile = str_glue("linkage_scripts/all_time_Dprime.log"), type="FORK")
...
#  .GlobalEnv$mat_filt <- mat_filt
  ...
  link_filt %>%
    fwrite(str_glue("results/linkage_out/observed_Dprime.temp/observed_Dprime.all_time.{idx}.gt1000.csv"))
