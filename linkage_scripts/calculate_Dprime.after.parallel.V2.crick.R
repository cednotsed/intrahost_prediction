...
parsed_agg <- fread(str_glue("results/allele_frequency_out/observed_out/after_{d}.aggregate.csv"))
...
# Get pAB
cl <- makeCluster(8, outfile = str_glue("linkage_scripts/after_{d}_Dprime.log"), type="FORK")
registerDoParallel(cl)

foreach(idx = seq(length(index_list)), 
#foreach(idx = seq(470, 500),
...
#  .GlobalEnv$mat_filt <- mat_filt
  ...
  link_filt %>%
    fwrite(str_glue("results/linkage_out/observed_Dprime.temp/observed_Dprime.after_{d}.{idx}.gt1000.csv"))
...
print(str_glue("Saved to: results/linkage_out/observed_Dprime.temp/observed_Dprime.after_{d}.{idx}.gt1000.csv"))
