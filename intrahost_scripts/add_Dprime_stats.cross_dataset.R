...
datasets <- c("alpha_to_ba1", "ba1_to_xbb",
              "xbb_to_end", "delta_to_ba5",
              "ba5_to_xbb", "pirola_to_end",
              "ba1_to_end", "ba1_to_ba5",
              "alpha_to_delta", "delta_to_ba1",
              "early_to_delta")

morsels <- foreach(dataset = datasets,
                   .packages = c("tidyverse", "data.table", "foreach")) %dopar% {
  # dataset = "early_to_delta"
  before <- str_split(dataset, "_")[[1]][1]
  
  # Mutation stats
  stat_df <- fread(str_glue("results/mutation_stats/{dataset}.stats.csv"))
  
  # Prior fitness
  before_df <- fread(str_glue("results/allele_frequency_out/observed_out/before_{before}.aggregate.csv"))
  
  # Master df
  cor_df <- fread(str_glue("results/linkage_out/intrahost_linkage.all/{before}.n5.with_zeroes.csv")) %>%
  ...
  obs_df <- fread(str_glue("results/linkage_out/observed_linkage/observed_linkage.before_{before}.gt1000.csv")) %>%
    mutate(mut1 = ifelse(mutation1 > mutation2, mutation1, mutation2),
           mut2 = ifelse(mutation1 > mutation2, mutation2, mutation1)) %>%
  ...
  dprime_df <- fread(str_glue("results/linkage_out/observed_linkage/observed_Dprime.before_{before}.gt1000.csv")) %>%
